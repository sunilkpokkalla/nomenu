...
# Process global-dish-library.ts
with open('src/lib/global-dish-library.ts', 'r') as f:
    content = f.read()

def replace_img(match):
    return f'"imageUrl": "{random.choice(images)}"'

# Replace all imageUrls in the dish library
new_content = re.sub(r'"imageUrl":\s*"[^"]+"', replace_img, content)

with open('src/lib/global-dish-library.ts', 'w') as f:
    f.write(new_content)

print("Updated global-dish-library.ts")

# Process branding-form.tsx
with open('src/components/dashboard/branding-form.tsx', 'r') as f:
    branding_content = f.read()

# Specifically replace the Equitaxe image
branding_content = branding_content.replace(
    'https://upload.wikimedia.org/wikipedia/commons/d/da/Equitaxe_-_20131111_-_Paris_-_01.jpg',
    random.choice(images)
)

with open('src/components/dashboard/branding-form.tsx', 'w') as f:
    f.write(branding_content)

print("Updated branding-form.tsx")
