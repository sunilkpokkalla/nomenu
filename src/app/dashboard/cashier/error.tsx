...
export default function CashierErrorBoundary({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    ...
    console.error("Cashier/POS Error Caught:", error);
  }, [error]);
  ...
  return (
    <div className="h-full min-h-[500px] w-full flex items-center justify-center bg-slate-50 p-6">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-sm border border-slate-200 p-8 flex flex-col items-center text-center">
        ...
        <h2 className="text-2xl font-bold text-slate-900 mb-2">POS Disconnected</h2>
        <p className="text-slate-500 mb-8">
          The Cashier system encountered an unexpected issue. Your orders are safely backed up in the cloud, but the display needs to be reloaded.
        </p>
