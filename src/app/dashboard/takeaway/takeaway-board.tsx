...
export function TakeawayBoard({ initialOrders, restaurantId, restaurantCreatedAt, timezone, supabaseUrl, supabaseAnonKey, isStandalone = false, locationLabel = "TABLE" }: { initialOrders: Order[], restaurantId: string, restaurantCreatedAt?: string | null, timezone: string, supabaseUrl: string, supabaseAnonKey: string, isStandalone?: boolean, locationLabel?: string }) {
  ...
  const fetchLatestOrders = async () => {
    try {
      ...
      if (error) {
        console.error("Error fetching latest takeaway orders:", error);
        return;
      }
      if (!data) return;
      ...
      console.error("Exception in fetchLatestOrders takeaway:", err);
    }
  };
    ...
    <div ref={containerRef} className={wrapperClasses}>
      ...
      <div className={`flex items-center justify-between ${isKdsMode ? "text-slate-100" : "text-slate-900"}`}>
        <h2 className="font-extrabold tracking-tight text-2xl flex items-center gap-3">
          ...
          {isKdsMode ? "KDS Live Terminal (Takeaway & Pickup)" : "Takeaway & Pickup Board"}
        </h2>
                ...
                <Droppable droppableId={col.id}>
                  {(provided, snapshot) => (
                    <div 
                      ...
                      className={`flex-1 overflow-y-auto ${col.id === "completed" || col.id === "cancelled" ? "grid grid-cols-2 auto-rows-max content-start" : "flex flex-col"} gap-3 rounded-xl transition-colors ${
