...
import { Copy, Check, Calendar, Mail, Search, MessageSquareCode } from "lucide-react";
...
export function MarketingTabs({ initialLeads }: MarketingTabsProps) {
  ...
  const handleCopyAll = () => {
    const emails = initialLeads.map(l => l.email).join(", ");
    ...
    <div className="space-y-6">
      ...
      <div className="flex border-b border-slate-200">
        ...
        >
          ...
          Nomi Leads ({initialLeads.length})
        </button>
      </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center justify-between bg-white border border-slate-200 p-5 rounded-xl shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">{initialLeads.length} total subscribers from Nomi Chatbot</p>
              </div>
            </div>
