...
export default async function LoginPage(
  props: {
    searchParams: Promise<{ message?: string; next?: string; email?: string; password?: string }>;
  }
) {
      ...
      <div className="flex-1 flex items-center justify-center p-4 sm:p-8 relative">
        ...
        <div className="w-full max-w-[480px] bg-white rounded-xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] p-8 sm:p-12 z-10">
          ...
          <div className="space-y-3 mb-6">
            <Link
              href={`/login?email=demo@nomenu.com&password=demo123${searchParams.next ? `&next=${encodeURIComponent(searchParams.next)}` : ""}`}
              className="w-full h-12 rounded-lg border border-transparent bg-[#1A73E8] hover:bg-[#1557B0] text-white font-medium flex items-center justify-center gap-3 text-base shadow-sm transition-colors duration-200"
            >
