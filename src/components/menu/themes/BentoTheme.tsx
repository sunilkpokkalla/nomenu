...
export function BentoTheme({ restaurant, categories, items, tableNumber, qrCodeId }: MenuThemeProps) {
  ...
  return (
    <div className="min-h-screen bg-[#F0FDF4] font-sans selection:bg-[#FFD166] selection:text-black">
      <div className="mx-auto w-full max-w-full sm:max-w-xl md:max-w-4xl lg:max-w-6xl xl:max-w-7xl min-h-screen bg-[#F0FDF4] sm:shadow-[8px_0_0_0_rgba(0,0,0,1)] sm:border-x-4 border-black flex flex-col pb-28 relative">
        
        {/* Vibrant Neo-brutalist Header */}
        <div className="bg-[#FFD166] border-b-4 border-black p-6 pt-10 sm:pt-12 relative overflow-hidden shrink-0">
          ...
          <div className="relative z-10 space-y-4">
            ...
            <h1 className="text-3xl sm:text-5xl font-black text-black leading-none drop-shadow-[2px_2px_0_rgba(0,0,0,1)] tracking-tight">
              {restaurant.name}
            </h1>
            ...
            <div className="flex gap-2 flex-wrap">
              {restaurant.phone && (
                <a href={`tel:${restaurant.phone}`} className="bg-white border-2 border-black px-3 py-1.5 text-xs font-bold shadow-[2px_2px_0_0_rgba(0,0,0,1)] flex items-center gap-1 hover:translate-y-[2px] hover:shadow-none transition-all">
                  <Phone className="w-3.5 h-3.5" /> Call
                </a>
              )}
              {restaurant.wifi_password && (
                <button onClick={() => setShowAmenities(true)} className="bg-white border-2 border-black px-3 py-1.5 text-xs font-bold shadow-[2px_2px_0_0_rgba(0,0,0,1)] flex items-center gap-1 hover:translate-y-[2px] hover:shadow-none transition-all">
        ...
        {/* Sticky Nav Bar */}
        <div className="sticky top-0 z-30 bg-[#F0FDF4] border-b-4 border-black py-3 px-4 space-y-3">
              ...
              return (
                ...
                >
                  <div className="inline-block bg-black text-white px-4 py-2 mb-6 transform -rotate-2">
                    <h2 className="text-xl sm:text-2xl font-black uppercase tracking-wider">{cat.name}</h2>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
                    {categoryItems.map((item) => {
                      ...
                      return (
                        ...
                        >
                          {item.image_url ? (
                            <div className="w-full aspect-video sm:aspect-square border-b-4 border-black bg-slate-100 relative overflow-hidden">
                              <Image 
                                src={item.image_url} 
                                alt={item.name} 
                            ...
                            <div className="w-full aspect-video sm:aspect-square border-b-4 border-black bg-[#118AB2] relative overflow-hidden flex items-center justify-center">
                               {item.is_popular && (
                                <div className="absolute top-2 -right-6 bg-[#FFD166] text-black text-[10px] font-black py-1 px-8 rotate-45 border-y-2 border-black">
                                  POPULAR
                                </div>
                          ...
                          <div className="p-4 flex flex-col flex-grow">
                            <div className="flex justify-between items-start gap-2 mb-1">
                              <h3 className="font-black text-base sm:text-sm text-black uppercase leading-tight line-clamp-2">
                                {item.name}
                              </h3>
                            </div>
                            <span className="font-black text-base sm:text-sm text-[#EF476F] mb-2">
                              {price}
                            </span>
                            
                            {item.description && (
                              <p className="text-xs sm:text-[10px] font-bold text-slate-600 line-clamp-2 leading-snug mt-auto">
                                {item.description}
                              </p>
                            )}
                          </div>
      ...
      {selectedItem && (
        <div className="fixed inset-x-0 bottom-0 sm:inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm p-0 sm:p-4 font-sans animate-in fade-in duration-300" onClick={() => setSelectedItem(null)}>
          <div className="relative bg-white w-full sm:max-w-md border-t-4 sm:border-4 border-black rounded-t-3xl sm:rounded-none shadow-[0_-10px_30px_rgba(0,0,0,0.3)] sm:shadow-[8px_8px_0_0_rgba(0,0,0,1)] flex flex-col max-h-[92vh]" onClick={(e) => e.stopPropagation()}>
            <div className="w-12 h-1.5 bg-black/20 rounded-full mx-auto my-2 sm:hidden"></div>
            <div className="p-4 border-b-4 border-black flex justify-between items-center bg-[#FFD166] rounded-t-2xl sm:rounded-none">
              <h2 className="text-xl font-black text-black uppercase">Item Details</h2>
              <button onClick={() => setSelectedItem(null)} className="text-black hover:bg-black hover:text-white p-1 border-2 border-transparent hover:border-black transition-colors rounded-lg">
                <X className="w-6 h-6" />
              </button>
            </div>
